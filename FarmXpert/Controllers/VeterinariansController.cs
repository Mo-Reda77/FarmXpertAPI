...
namespace FarmXpertLogin.Controllers
{
    ...
    public class VeterinariansController : ControllerBase
    {
        ...
        public async Task<IActionResult> UpdateVeterinar(int id, [FromBody] UpdateWorker model)
        {
            var managerFarmId = int.Parse(User.FindFirst("FarmId")?.Value ?? "0");
            ...
            if (!string.IsNullOrEmpty(model.ImagePath))
                veterinar.ImagePath = model.ImagePath;

            await _FarmDb.SaveChangesAsync();

            return Ok(new
            {
                message = "Veterinar updated successfully",
                veterinar = new
                {
                    veterinar.Id,
                    veterinar.Name,
